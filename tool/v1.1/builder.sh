...
# LAST Modified: 15:49:28 19-05-2026
# DESCRIPTION:
...
mkdir -p "$FP_DIR" "$FP_DIR_COMBINED" "$FP_DIR_FLAT" "$SYM_DIR" "$SYM_DIR_FLAT" "$MODEL_DIR"

#######################################
# Footprints
#######################################
echo ""
# Find the files themselves
...
# Calculate the number of directories found
...
if (( num_fp_dir > 0 )); then
  copy_footprints_dirs "$FP_DIR_FLAT" "${footprint_dirs[@]}"
else
